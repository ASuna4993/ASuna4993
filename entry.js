...
// Googleフォームのエントリーコード
const FORM_ENTRIES = {
    EMPLOYEE_NAME: 'entry.499409665',
    EMPLOYEE_BRANCH: 'entry.1577372347',
    NAME: 'entry.2051556631',
    AGE: 'entry.1557022610',
    GENDER: 'entry.1622090297',
    EMAIL: 'entry.1258305063',
    PHONE: 'entry.1971705522',
    LOCATION: 'entry.103579559',
    NOTES: 'entry.1932797490',
    CONSENT: 'entry.1858117721'
};
...
function setFormEntries() {
    ...
    // 性別（ラジオボタン）
    document.querySelectorAll('#gender_input input[type="radio"]')
        .forEach(radio => radio.name = FORM_ENTRIES.GENDER);
    
    document.getElementById('email_body').name = FORM_ENTRIES.EMAIL;
    document.getElementById('phone_body').name = FORM_ENTRIES.PHONE;
    document.getElementById('location_body').name = FORM_ENTRIES.LOCATION;
...
function setParams(params) {
    ...
    setFormValue("location_body", params.location);
...
function confirmForm(appType) {
    ...
    // 年齢のバリデーション
    const age = document.getElementById("age_body").value;
    ...
    // 希望勤務地のバリデーション
    if (document.getElementById("location_body").value === '') {
